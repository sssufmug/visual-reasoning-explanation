...
# from Concept_Discovery.ace_helpers import *
...
class ConceptDiscovery(object):
  ...
  def create_patches(self, method='slic', discovery_images=None,
    ...
    if self.num_workers:
      ...
      if gradcam:
        outputs = pool.map(
          lambda img: self._return_gradcam_superpixels(img, method, param_dict),
  ...
  def _return_superpixels(self, img, method='slic',
    ...
    superpixels, patches = [], []
    '''save the unique mask for later training'''
    self.mask = np.array(unique_masks)
  ...
  def _return_gradcam_superpixels(self, img, method='slic',
                          param_dict=None):
    """Returns all patches for one image.
    ...
    cam = gradCAM.compute_heatmap(inputs, chosen_class, upsample_size, 50)[1][:,:,0]

    """
    Ava
    from src.gradcam import get_preds, decode_predictions
    source_dir = '/lab/tmpig23b/u/andy/VR_SC/ACEdata/results/output'
    class_dict = {
      'ambulance': 'n02701002',
      'school_bus': 'n04146614',
      'police_van': 'n03977966'}
    img, x, preds = get_preds(self.model.model)
    class_name = decode_predictions(preds, top=len(preds[0]))[0]
    idx = np.argsort(preds[0])[::-1]
    labels = {}
    for i in range(1000):
        labels[class_name[i][1]] = str(idx[i])

    for target_class, filder in class_dict.items():
        target_class_path = os.path.join(source_dir, target_class, 'concepts/images')
        for roots, dirs, files in os.walk(target_class_path):
            for i, file in enumerate(files):
                IMAGE_PATH = os.path.join(roots, file)
                img, x, preds = get_preds(self.model.model, IMAGE_PATH)
                print(target_class+' Predicted:', decode_predictions(preds, top=1)[0])
                upsample_size = (img.shape[1], img.shape[0])
                label = int(labels[target_class])
                gradCAM.showCAMs(img, x, label, upsample_size)
                if i == 25:
                    break
            else:
                break
    ...
    """
    if sum(sum(cam)) == 0:
      return self._return_superpixels(img, method, param_dict)
    if method == 'slic':
      n_segmentss = param_dict.pop('n_segments', [15, 50, 80])
      n_params = len(n_segmentss)
      compactnesses = param_dict.pop('compactness', [20] * n_params)
  ...
  def match(self,
    ...
    for bn in self.bottlenecks:
      bn_dic = {}
      # step 1 get the vectore for each patch
      if activations is None or bn not in activations.keys():
        bn_activations = self._patch_activations(self.dataset, bn)
      else:
        bn_activations = activations[bn]
    # step 2 get the prior
      ...
      center_match = {}
      center_match_vector = {}
      for key in prior[bn].keys():
        if 'center' in key:
          center_dict[key] = prior[bn][key]
      for concept_name, center_value in center_dict.items():
        # computer the MES for all patches
        err = np.sum(np.square(bn_activations - center_value), axis=1)
        index = np.argmin(err, axis=0)
        center_match[concept_name] = index
        center_match_vector[concept_name] = bn_activations[index]
        # index in dataset match to this current concept
      self.center_match = center_match
      self.center_match_vector = center_match_vector

  def match_dummy(self,
                        prior,
                        target,
                        method='KM',
                        activations=None,
    ...
    for bn in self.bottlenecks:
      ...
      for key in prior[bn].keys():
        if 'center' in key:
          concept_index = key.split('_')[-2].split('concept')[1]
          if int(concept_index) in target: # get the concept number 'ambulance_concept1_center'
            center_dict[concept_index] = prior[bn][key]
      used_concept_index = []
      for con_index in target:
        center_value = center_dict[str(con_index)]
      # for concept_name, center_value in center_dict.items():
      #   # computer the MES for all patches
        err = np.sum(np.square(bn_activations - center_value), axis=1)
        error_list = list(np.argsort(err)) # from small to large
        for index_can in error_list:
          if index_can not in used_concept_index: # not duplicate
            img_index = index_can
            used_concept_index.append(index_can)
            break
        center_match[str(con_index)] = img_index
        center_match_vector[str(con_index)] = bn_activations[img_index]
        center_match_error[str(con_index)] = err[img_index]
        # index in dataset match to this current concept
      self.center_match = center_match
      self.center_match_vector = center_match_vector
      self.center_match_error = center_match_error
  def match_topk(self,
                        prior,
                        topk,
                        method='KM',
    ...
    # step 2 get the prior
      ...
      for concept_name, center_value in center_dict.items():
        ...
        err = np.sum(np.square(bn_activations - center_value), axis=1)
        # index = np.argmin(err, axis=0)
        index = err.argsort()[0:topk] #output topk
        center_match[concept_name] = index
        # index in dataset match to this current concept
      self.center_match = center_match
